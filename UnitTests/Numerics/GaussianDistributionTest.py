...
class GaussianDistributionTest(unittest.TestCase):
    ...
    def testCumulativeTo(self):
        ...
        answer = Gauss.cumulative_to(0.5)
    ...
    def testMultiplication(self):
        standard_normal = Gauss(0, 1)
        shifted_gaussian = Gauss(2, 3)
    ...
    def testDivision(self):
        product = Gauss(0.2, 3.0 / sqrt(10))
        standard_normal = Gauss(0.0, 1.0)
        ...
        product2 = Gauss((4.0 * 7.0 ** 2 + 6.0 * 5.0 ** 2) / (5.0 ** 2 + 7.0 ** 2), sqrt((5.0 ** 2 * 7.0 ** 2) / (5.0 ** 2 + 7.0 ** 2)))
        m4s5 = Gauss(4.0, 5.0)
    ...
    def testLogProductNormalization(self):
        standard_normal = Gauss(0, 1)
        lpn = Gauss.log_product_normalization(standard_normal, standard_normal)
        ...
        m1s2 = Gauss(1.0, 2.0)
        m3s4 = Gauss(3.0, 4.0)
        lpn2 = Gauss.log_product_normalization(m1s2, m3s4)
    ...
    def testLogRatioNormalization(self):
        m1s2 = Gauss(1.0, 2.0)
        m3s4 = Gauss(3.0, 4.0)
        lrn = Gauss.log_ratio_normalization(m1s2, m3s4)
