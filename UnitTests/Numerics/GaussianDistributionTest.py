...
class GaussianDistributionTest(unittest.TestCase):
    ...
    def testCumulativeTo(self):
        ...
        answer = GaussianDistribution.cumulative_to(0.5)
    ...
    def testAt(self):
        ...
        answer = GaussianDistribution.at(0.5)
        self.assertAlmostEqual(expected, answer, None,
                               "testAt expected %.15f, got %.15f" % (expected, answer),
                               GaussianDistributionTest.ERROR_TOLERANCE)
    
    def testMultiplication(self):
        standard_normal = GaussianDistribution(0, 1)
        shifted_gaussian = GaussianDistribution(2, 3)
    ...
    def testDivision(self):
        ...
        standard_normal = GaussianDistribution(0.0, 1.0)
        ...
        product2 = GaussianDistribution((4.0 * 7.0 ** 2 + 6.0 * 5.0 ** 2) / (5.0 ** 2 + 7.0 ** 2), sqrt((5.0 ** 2 * 7.0 ** 2) / (5.0 ** 2 + 7.0 ** 2)))
        m4s5 = GaussianDistribution(4.0, 5.0)
    ...
    def testLogProductNormalization(self):
        standard_normal = GaussianDistribution(0, 1)
        lpn = GaussianDistribution.log_product_normalization(standard_normal, standard_normal)
        ...
        m1s2 = GaussianDistribution(1.0, 2.0)
        m3s4 = GaussianDistribution(3.0, 4.0)
        lpn2 = GaussianDistribution.log_product_normalization(m1s2, m3s4)
    ...
    def testLogRatioNormalization(self):
        m1s2 = GaussianDistribution(1.0, 2.0)
        m3s4 = GaussianDistribution(3.0, 4.0)
        lrn = GaussianDistribution.log_ratio_normalization(m1s2, m3s4)
